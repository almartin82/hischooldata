# ==============================================================================
# LIVE Pipeline Tests for hischooldata
# ==============================================================================
#
...
test_that("Hawaii DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add Hawaii DOE URL and verify HTTP 200")
})
...
test_that("Can download Hawaii enrollment data file", {
  skip_if_offline()
...
test_that("Can parse Hawaii enrollment file", {
  skip_if_offline()
...
test_that("hischooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- hischooldata::get_available_years()
  ...
  tryCatch({
    raw <- hischooldata:::get_raw_enr(test_year)
...
test_that("get_available_years returns valid year range", {
  result <- hischooldata::get_available_years()
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- hischooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- hischooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- hischooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- hischooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- hischooldata:::get_cache_path(2024, "enrollment")
